...
summarize(activities = paste(sort(unique(activity)),collapse=", ")) %>% na.omit() -> activity_text
...
#------------------------------------------------------------------------------
activities <- activity_congregations %>% mutate(link_type = "activity", color = "yellow") %>% relocate(color, link_type, activity) %>% rename(specific_link = activity) %>% rev()

partners <- partner_congregations %>% mutate(link_type = "partner", color = "pink") %>% relocate(color, link_type, partner) %>% rename(specific_link = partner) %>% rev()
